...
class CustomToolTipRender extends React.Component {
	...
	handleOpenPopup = event => {
		...
		return this.setState({textData: this.props.cell.rowInfo.getData()[this.props.column.dataField.replace('Count', 'Str')]})
	...
	render() {
		return (
			<div onMouseEnter={() => this.setState({ visible: 'visible' })} onMouseLeave={() => this.setState({ visible: 'hidden' })} style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', columnGap: '10px', margin: '3px' }}>
				{this.props.column.dataField.includes('ClaimPaymentCount')? this.props.row.getData()[this.props.column.dataField] : null}
				...
				{this.props.column.dataField === 'meac_hbClaimPaymentCount' ? this.props.row.getData().meac_hbClaimPaymentCount : null}
				{this.props.column.dataField === 'meac_pbClaimPaymentCount' ? this.props.row.getData().meac_pbClaimPaymentCount : null} */}
