    ...
    render() {
        return (
            <Paper style={{ background: 'linear-gradient(to left, white, #c0cec6, white, #c0cec6, white, #c0cec6, white, #c0cec6, white)', height: '800px', marginTop: '2px' }}>
                ...
                <div className="deliveryControlGrid">
                    <DataGrid id="grid" ref={g => (this.grid = g)} width="100%" height="100%" enableCopy={true} alternatingItemColors={[0xe1e8e4, 0xffffff]} styleName="gridStyle" enableEagerDraw={true}>
                        <ReactDataGridColumnLevel rowHeight="20" enableFilters={true} enablePaging={true} pageSize="50">
                            ...
                            <ReactDataGridColumn dataField="activeFlag" enableCellClickRowSelect={false} width="200" headerText="Active" headerAlign="center" itemRenderer={new ClassFactory(DeliveryActiveRenderer)}
                                onHandleActivatePoll={(e, data) =>
                                    this.activateDelivery(e, data)
                                }
                            />
                            <ReactDataGridColumn sortable={false} enableCellClickRowSelect={false} width="200" headerAlign="center" headerText="Dispatch Delivery" itemRenderer={new ClassFactory(DispatchDeliveryRenderer)}
                                onHandleDeliver={(e, data) =>
                                    this.deliver(e, data)
                                }
                            />
                            <ReactDataGridColumn
                ...
                <AdvanceDialog
                    open={this.state.deliveryEditor}
                    handleClose={() =>
                        this.setState({ deliveryEditor: false })
                    }
                    bodyRenderer={
                        <DeliveryEditor />
                    }
