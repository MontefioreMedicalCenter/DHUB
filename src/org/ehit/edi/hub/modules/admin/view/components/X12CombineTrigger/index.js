...
class X12CombineTrigger extends EventDispatcher {
	...
	viewStagedFiles = data => {
		...
		window.open('ftp://ediprod:xhub9@ediprod' + stagedLink, '_blank')
	}
	render() {
		return (
					...
					<DataGrid id="grid" ref={g => (this.grid = g)} width="100%" height="100%" enableCopy={true} enableEagerDraw={true} styleName="gridStyle" alternatingItemColors={[0xe1e8e4, 0xffffff]}>
						<ReactDataGridColumnLevel rowHeight="21" enableFilters={true} enablePaging={true} pageSize="50">
							...
							<ReactDataGridColumn
								...
								headerText="Active"
								headerAlign="center"
								itemRenderer={new ClassFactory(TriggerActiveRenderer)}
								onHandleActivatePoll={(event, data) => {
									this.activatePoll(event, data)
								}}
							/>
							<ReactDataGridColumn
								dataField="stagedLink"
								sortable={false}
								enableCellClickRowSelect={false}
								width="200"
								headerText="Staged Files"
								headerAlign="center"
								itemRenderer={new ClassFactory(StagedLinkRender)}
							...
							<ReactDataGridColumn
								...
								enableCellClickRowSelect={false}
								width="200"
								headerText="Combine X12"
								headerAlign="center"
								itemRenderer={new ClassFactory(TriggerRenderer)}
								onHandlePoll={(e, data) => {
									this.poll(e, data)
								}}
							/>
