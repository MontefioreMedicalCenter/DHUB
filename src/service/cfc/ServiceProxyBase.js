...
import { watch as networkStateWatcher, check as checkNetworkStatus } from 'is-offline'
...
export default class ServiceProxyBase extends TypedObject {
	constructor() {
		...
		this.baseUrl = window.location.href.indexOf('localhost') >= 0 || window.location.href.indexOf('192.168.') >= 0 ? 'http://localhost:8080/' : window.location.origin + '/'
	}
	...
	callServiceMethod = (method, urlPath, args, token, resultFunction, faultFunction, contentType, header) => {
		...
		let promise = this.callApi(method, urlPath, token, args, contentType, header)
		promise.then(
			response => {
				...
				if (resultFunction) {
					let result
					try {
						result = typeof response.data === 'string' && response.data.length > 0 && response.data.includes('{') ? JSON.parse(response.data) : response.data
		...
		if (ServiceProxyBase.IsOffline) {
			promise[this.useJsonService ? 'reject' : 'resolve'](new amf.Response(503, 'No network connection'))
		}
