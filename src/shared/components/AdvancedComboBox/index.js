...
import { Button, Checkbox, FormControl, FormGroup, InputLabel, MenuItem, NativeSelect, Select, Paper, FormHelperText } from '@material-ui/core'
...
const OkCancelPaperComponent = withStyles(theme => ({
...
}))(({ children, classes, maxHeight, enableOkCancel, onCancel, onOk, listRef, ...otherProps }) => {
	...
	const OkCancelButtons = enableOkCancel ? (
		<HBox className={classes.okCancelContainer} justify="flex-end">
			<FormGroup row style={{ width: 'fit-content' }}>
				<Button key={'cancel'} style={{ pointerEvents: 'all' }} onClick={handleOnCancel} color="default">
					Cancel
				</Button>
				<Button key={'okay'} style={{ pointerEvents: 'all' }} onClick={handleOnOk} color="primary">
					OK
				</Button>
			</FormGroup>
		</HBox>
	...
	return (
		<Paper {...otherProps}>
			<div ref={listRef} style={{ overflowY: 'auto', overflowX: 'hidden', maxHeight }}>
				{children}
			</div>
			{OkCancelButtons}
		</Paper>
...
class AdvancedComboBox extends React.Component<Props> {
	...
	constructor(props: Props) {
		super(props)
		this.state = {
			selected: props.selectedItems || (props.selectedItem ? [props.selectedItem] : []),
			changed: false,
			selectAll: true,
			open: false,
			rootKey: UUID(),
	...
	toggleSelect = (event, value) => {
		...
		const currentIndex = newSelected ? newSelected.findIndex(item => item[data] === value[data]) : -1
	...
	handleOnSelectAll = (event, value) => {
		...
		const { enableMultiSelect } = this.props
		let newSelected = enableMultiSelect === true && selected ? [...selected] : []
	...
	handleOnChange = (event: Event) => {
		...
		const source = this.props.enableNativeMode ? event.currentTarget.selectedOptions[0] : event.currentTarget
	...
	handleOnKeyUp = event => {
		...
		const matched = dataProvider.filter(item => item[dataLabel] && item[dataLabel].toLowerCase().indexOf(key) === 0)

		const stateObj = { changed: true }
		if (matched.length) {
			stateObj.nextMatchIndex = lastPressedKey === key && matched.length - 1 > nextMatchIndex ? nextMatchIndex + 1 : 0
	...
	isChecked = (items, item) => {
		if (items) {
			for (let i = 0; i < items.length; i++) {
				if (items[i] === item || items[i][this.props.data] === item[this.props.data]) {
					return true
				}
			}
		}
	...
	focusListItem = focusedItem => {
		...
		if (focusedItem && this.menuListEl) {
			const listItem = this.menuListEl.querySelector('[data-id="' + dataProvider.indexOf(focusedItem) + '"]')
			if (listItem) {
				listItem.focus()
			}
		}
	...
	renderSelectedValues = selected => {
		...
		if (showSelectAll) {
			if (selectAll) {
				...
				return emptySelect ? (emptySelect === true ? 'Select' : emptySelect) : ''
			}
		}
	...
	renderOptions = (item, index) => {
		const { classes, data, dataLabel, enableMultiSelect, enableNativeMode } = this.props
		...
		return enableNativeMode ? (
			<option key={index} value={value} data-id={index} style={{ ...buttonColor }}>
				{item[dataLabel]}
			</option>
		) : (
			<MenuItem
				...
				{enableMultiSelect && <Checkbox checked={this.isChecked(selected, item)} color="primary" tabIndex={-1} disableRipple className={classes.zeroPadding} />}
				{item[dataLabel]}
			</MenuItem>
		)
	}
	...
	renderNativeSelect = () => {
		const { classes, data, dataProvider, emptySelect, enableMultiSelect, fullWidth, label, fixedTheme, selectInlineStyle } = this.props
		...
		return (
			<NativeSelect
				value={selected ? selected.map(item => item[data]).join(', ') : undefined}
				...
				{emptySelect && <option value="">{typeof emptySelect === 'boolean' ? 'Select' : emptySelect}</option>}
				{(dataProvider || []).map(this.renderOptions)}
			</NativeSelect>
		)
	}
	handleFocus = () => {
		if (this.inputRef && this.inputRef.node && this.inputRef.node.previousSibling) this.inputRef.node.previousSibling.focus()
	}
	renderSelect = () => {
		const { classes, data, dataLabel, dataProvider, emptySelect, showSelectAll, enableMultiSelect, fullWidth, fixedTheme, outline, selectInlineStyle, SelectProps } = this.props
		const { selected } = this.state
		...
		if (emptySelect) {
			emptyItem = {}
			emptyItem[data] = NONE
			emptyItem[dataLabel] = typeof emptySelect === 'boolean' ? 'Select' : emptySelect
			emptyItem = [emptyItem]
		}
		const cbx = (
			<Select
				...
				{showSelectAll && (
					<MenuItem value={0} key={`${this.state.selectAll}`}>
						<Checkbox color="primary" checked={this.state.selectAll} value={this.state.selectAll} tabIndex={-1} disableRipple className={this.props.classes.zeroPadding} />
						{'Select All'}
					</MenuItem>
				)}
				{(dataProvider || []).map(this.renderOptions)}
		...
		return (
			<React.Fragment>
				{this.props.focusable ? (
					<div>
						<input type={'text'} style={{ width: '4px', height: '4px' }} onFocus={this.handleFocus}></input>
						{cbx}{' '}
					</div>
				) : (
					cbx
	...
	render = () => {
		const { classes, disabled, outline, isRequired, enableNativeMode, OutlinedComponentProps, fullWidth, label, fixedTheme, enableShrink = true } = this.props

		const children = [
			<div key={this.state.rootKey} className={classNames(classes.root, { [Styles.fullWidth]: fullWidth })}>
				<FormControl
		...
		return (
			<div className={classNames({ [Styles.disabled]: disabled })}>
				...
				{isRequired && <FormHelperText style={{ padding: '0px 0px 0px 18px' }}>*Required</FormHelperText>}
			</div>
		)
	}
}
