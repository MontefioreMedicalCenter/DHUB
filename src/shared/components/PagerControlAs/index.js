...
export default class PagerControl extends UIComponent {
	constructor() {
		super({}, 'span')
		this.addEventListener(this, Constants.EVENT_CLICK, function(e) {
	...
	onPageChanged() {
		...
		if (pageDropDown && pageDropDown.value != this._pageIndex) {
			pageDropDown.value = (this._pageIndex + 1).toString()
		}
		if (this.doDispatchEvents) this.dispatchEvent(new flexiciousNmsp.ExtendedFilterPageSortChangeEvent(flexiciousNmsp.ExtendedFilterPageSortChangeEvent.PAGE_CHANGE))
	}
	...
	getPageDropdown() {
		return UIUtils.findFirstElementByTagName(this.domElement, 'select')
	}

	destroy() {
		this.destroyButtons([PagerControl.ACTION_FIRST_PAGE, PagerControl.ACTION_FIRST_PAGE, PagerControl.ACTION_PREV_PAGE, PagerControl.ACTION_NEXT_PAGE, PagerControl.ACTION_LAST_PAGE, PagerControl.ACTION_SORT, PagerControl.ACTION_SETTINGS, PagerControl.ACTION_SAVE_SETTINGS, PagerControl.ACTION_FILTER_SHOW_HIDE, PagerControl.ACTION_RUN_FILTER, PagerControl.ACTION_CLEAR_FILTER, PagerControl.ACTION_PRINT, PagerControl.ACTION_PDF, PagerControl.ACTION_WORD, PagerControl.ACTION_EXCEL])
		const pageDropDown = this.getPageDropdown()
		if (pageDropDown) {
	...
	addToolbarActionsHTML() {
		...
		for (const tca of this.grid.toolbarActions) {
			html.push(
				tca.seperatorBefore ? (
					<span id={gridId + '_' + tca.code} className={'pagerDiv separatorCell'}>
						|
					</span>
				) : (
					''
			...
			html.push(
				<span valign={'middle'} style={style}>
	...
	render() {
				...
				topLevelToolbarButtons.push(
					<span key="2">
						<span key={gridId + 'btnSort'} id={gridId + 'btnSort'} className={'pagerDiv  iconCell'}>
							<img tabIndex={0} src={this.grid.getThemeToolbarIconFolder() + '/sort.png'} className={'imageButtonSort'} alt={Constants.PGR_BTN_SORT_TOOLTIP} title={Constants.PGR_BTN_SORT_TOOLTIP} />
						</span>
						<span key={gridId + 'linesep' + linesep++} className={'pagerDiv lineSep'}>
							&nbsp;
						</span>
			...
			if (this.grid.enablePreferencePersistence) {
				topLevelToolbarButtons.push(
					<span key="3">
						<span key={gridId + 'btnSettings'} id={gridId + 'btnSettings'} className={'pagerDiv  iconCell'}>
							<img tabIndex={0} src={this.grid.getThemeToolbarIconFolder() + '/settings.png'} className={'imageButtonSettings'} alt={Constants.PGR_BTN_SETTINGS_TOOLTIP} title={Constants.PGR_BTN_SETTINGS_TOOLTIP} />
						</span>
					</span>
				)
				if (this.grid.enableMultiplePreferences) {
					topLevelToolbarButtons.push(
						<span key="4">
							<span key={gridId + 'btnSettings'} id={gridId + 'btnSettings'} className={'pagerDiv  iconCell'}>
								<img tabIndex={0} src={this.grid.getThemeToolbarIconFolder() + '/openSettings.png'} className={'imageButtonOpenSettings'} alt={Constants.PGR_BTN_OPEN_SETTINGS_TOOLTIP} title={Constants.PGR_BTN_OPEN_SETTINGS_TOOLTIP} />
							</span>
						</span>
					)
				}
				...
				topLevelToolbarButtons.push(
					<span key="5">
						<span key={gridId + 'btnSaveSettings'} id={gridId + 'btnSaveSettings'} className={'pagerDiv  iconCell'}>
							<img tabIndex={0} src={this.grid.getThemeToolbarIconFolder() + '/saveSettings.png'} className={'imageButtonSaveSettings'} alt={Constants.PGR_BTN_SAVE_SETTINGS_TOOLTIP} title={Constants.PGR_BTN_SAVE_SETTINGS_TOOLTIP} />
						</span>
						<span key={gridId + 'linesep' + linesep++} className={'pagerDiv lineSep'}>
							&nbsp;
						</span>
					...
					<span key="6">
						<span key={gridId + 'btnFilterShowHide'} id={gridId + 'btnFilterShowHide'} className={'pagerDiv  iconCell'}>
							<Tooltip title={Constants.PGR_BTN_FILTER_TOOLTIP}>
								<img tabIndex={0} src={filterShowHide} className={'imageButtonFilterShowHide'} alt={Constants.PGR_BTN_FILTER_TOOLTIP} style={{ marginTop: '9px' }} />
							</Tooltip>
						</span>
					</span>
				)
					...
					<span key="7">
						<span key={gridId + 'btnFilter'} id={gridId + 'btnFilter'} className={'pagerDiv  iconCell'}>
							<Tooltip title={Constants.PGR_BTN_RUN_FILTER_TOOLTIP}>
								<img tabIndex={0} src={filter} className={'imageButtonFilter'} alt={Constants.PGR_BTN_RUN_FILTER_TOOLTIP} style={{ marginTop: '9px' }} />
							</Tooltip>
						</span>
					</span>
				)
					...
					<span key="8">
						<span key={gridId + 'btnClearFilter'} id={gridId + 'btnClearFilter'} className={'pagerDiv  iconCell'}>
							<Tooltip title={Constants.PGR_BTN_CLEAR_FILTER_TOOLTIP}>
								<img tabIndex={0} src={clearFilter} className={'imageButtonClearFilter'} alt={Constants.PGR_BTN_CLEAR_FILTER_TOOLTIP} style={{ marginTop: '9px' }} />
							</Tooltip>
						</span>
						<span key={gridId + 'linesep' + linesep++} className={'pagerDiv lineSep'}>
							&nbsp;
				...
				topLevelToolbarButtons.push(
					<span key="9">
						<span key={gridId + 'btnPrint'} id={gridId + 'btnPrint'} className={'pagerDiv  iconCell'}>
							<img tabIndex={0} src={this.grid.getThemeToolbarIconFolder() + '/print.png'} className={'imageButtonPrint'} alt={Constants.PGR_BTN_PRINT_TOOLTIP} title={Constants.PGR_BTN_PRINT_TOOLTIP} />
						</span>
					</span>
				)
			}
				...
				topLevelToolbarButtons.push(
					<span key="10">
						<span key={gridId + 'btnPdf'} id={gridId + 'btnPdf'} className={'pagerDiv  iconCell'}>
							<img tabIndex={0} src={this.grid.getThemeToolbarIconFolder() + '/pdf.png'} className={'imageButtonPdf'} alt={Constants.PGR_BTN_PDF_TOOLTIP} title={Constants.PGR_BTN_PDF_TOOLTIP} />
						</span>
					</span>
				)
			}
					...
					<span key="11">
						<span key={gridId + 'btnWord'} id={gridId + 'btnWord'} className={'pagerDiv  iconCell'}>
							<Tooltip title={Constants.PGR_BTN_WORD_TOOLTIP}>
								<img tabIndex={0} src={word} className={'imageButtonWord'} alt={Constants.PGR_BTN_WORD_TOOLTIP} style={{ paddingBottom:"10px", paddingTop:"10px" }} />
							</Tooltip>
						</span>
					</span>
				)
		...
		const val = (
			<div className={'pagerControl flexiciousGridPager cellRenderer'} style={{ display: 'block' }}>
				<span className={'pagerTable'} key={gridId + 'pagerTable'} style={{ float: 'left', height: this.getHeight() + 'px' }}>
					...
					{this.level.enablePaging ? (
						<span key={gridId + 'btnFirstPage'} id={gridId + 'btnFirstPage'} className={'pagerDiv iconCell firstPage'}>
							<img alt="First Page" tabIndex="0" src={firstPageArrow} className={'imageButtonFirstPage'} alt={Constants.PGR_BTN_FIRST_PAGE_TOOLTIP} title={Constants.PGR_BTN_FIRST_PAGE_TOOLTIP} style={{ marginTop: '9px' }} />
						</span>
					) : null}

					{this.level.enablePaging ? (
						<span key={gridId + 'btnPreviousPage'} id={gridId + 'btnPreviousPage'} className={'pagerDiv iconCell prevPage'}>
							<img alt="Previous Page" tabIndex="0" src={prevPage} className={'imageButtonPrevPage'} alt={Constants.PGR_BTN_PREV_PAGE_TOOLTIP} title={Constants.PGR_BTN_PREV_PAGE_TOOLTIP} style={{ marginTop: '9px' }} />
						</span>
					) : null}

					{this.level.enablePaging ? (
						<span key={gridId + 'btnNextPage'} id={gridId + 'btnNextPage'} className={'pagerDiv iconCell nextPage'}>
							<img alt="Next Page" tabIndex="0" src={nextPage} className={'imageButtonNextPage'} alt={Constants.PGR_BTN_NEXT_PAGE_TOOLTIP} title={Constants.PGR_BTN_NEXT_PAGE_TOOLTIP} style={{ marginTop: '9px' }} />
						</span>
					) : null}

					{this.level.enablePaging ? (
						<span key={gridId + 'btnLastPage'} id={gridId + 'btnLastPage'} className={'pagerDiv iconCell lastPage'}>
							<img alt="Next Page" tabIndex="0" src={lastPage} className={'imageButtonLastPage'} alt={Constants.PGR_BTN_LAST_PAGE_TOOLTIP} title={Constants.PGR_BTN_LAST_PAGE_TOOLTIP} style={{ marginTop: '9px' }} />
						</span>
					) : null}
					...
					{this.level.enablePaging ? (
						<span key={gridId + 'lblGotoPage'} id={gridId + 'lblGotoPage'} className={'pagerDiv  gotoPage'}>
							{flexiciousNmsp.Constants.PGR_LBL_GOTO_PAGE_TEXT} <select className={'pageDropdown'}> </select>{' '}
						</span>
					) : null}
	...
	componentDidMountCustom() {
		this.initializeButtons([PagerControl.ACTION_FIRST_PAGE, PagerControl.ACTION_FIRST_PAGE, PagerControl.ACTION_PREV_PAGE, PagerControl.ACTION_NEXT_PAGE, PagerControl.ACTION_LAST_PAGE, PagerControl.ACTION_SORT, PagerControl.ACTION_SETTINGS, PagerControl.ACTION_OPEN_SETTINGS, PagerControl.ACTION_SAVE_SETTINGS, PagerControl.ACTION_OPEN_SETTINGS, PagerControl.ACTION_FILTER_SHOW_HIDE, PagerControl.ACTION_RUN_FILTER, PagerControl.ACTION_CLEAR_FILTER, PagerControl.ACTION_PRINT, PagerControl.ACTION_PDF, PagerControl.ACTION_WORD, PagerControl.ACTION_EXCEL, PagerControl.ACTION_EXPAND_UP, PagerControl.ACTION_EXPAND_ALL, PagerControl.ACTION_EXPAND_DOWN, PagerControl.ACTION_COLLAPSE_ALL])
		const pageDropDown = this.getPageDropdown()
		if (!this.level) {
			return
		}
	...
	enableDisableButton(button, enabled) {
		...
		} else UIUtils.detachClass(button, 'disabled')
	}
	...
	refresh() {
		...
		if(this.getPageCount() > "0"){
			if (pageInfo) pageInfo.innerHTML = `${Constants.PGR_ITEMS} ${this.getPageStart()} ${Constants.PGR_TO} ${this.getPageEnd()} ${Constants.PGR_OF} ${this.getTotalRecords()}. ${Constants.PGR_PAGE} ${this.getPageIndex() + 1} ${Constants.PGR_OF} ${this.getPageCount()} `
		}else{
			if (pageInfo) pageInfo.innerHTML = `${Constants.PGR_ITEMS} ${this.getPageStart()} ${Constants.PGR_TO} ${this.getPageEnd()} ${Constants.PGR_OF} ${this.getTotalRecords()}. ${Constants.PGR_PAGE} ${this.getPageIndex()} ${Constants.PGR_OF} ${this.getPageCount()} `
		}
		const firstPage = UIUtils.findElementWithClassName(this.domElement, 'firstPage')
		if (firstPage) this.enableDisableButton(firstPage, this.getPageIndex() > 0)
		const prevPage = UIUtils.findElementWithClassName(this.domElement, 'prevPage')
		if (prevPage) this.enableDisableButton(prevPage, this.getPageIndex() > 0)
		const nextPage = UIUtils.findElementWithClassName(this.domElement, 'nextPage')
		if (nextPage) this.enableDisableButton(nextPage, this.getPageIndex() < this.getPageCount() - 1)
		const lastPage = UIUtils.findElementWithClassName(this.domElement, 'lastPage')
		if (lastPage) this.enableDisableButton(lastPage, this.getPageIndex() < this.getPageCount() - 1)
