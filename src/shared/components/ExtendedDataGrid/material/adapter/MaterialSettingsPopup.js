...
export default class MaterialSettingsPopup extends UIComponent {
	...
	showDialog() {
		const actions = [ToolbarAction.create(Constants.MCS_BTN_APPLY_LABEL, this.onOK.bind(this), true), ToolbarAction.create(Constants.MCS_BTN_CANCEL_LABEL, this.onCancel.bind(this), true)]
			...
			<div style={{ width: 540 }}>
				...
				<div style={{ marginLeft: 15, width: 500 }}>
					<MaterialDataGrid
						...
						height={300}
						selectedObjects={this._cols.length === this._visibleCols.length ? this._cols : this._visibleCols}
						width={'100%'}
						onChange={evt => {
							this.selectedColumns = evt.grid.getSelectedObjects()
						}}>
						<MaterialCheckBoxColumn />
						<MaterialDataGridColumn dataField={'uniqueIdentifier'} headerText={Constants.SETTINGS_COLUMNS_TO_SHOW} labelFunction={this.getHeaderText} />
					</MaterialDataGrid>
				</div>
