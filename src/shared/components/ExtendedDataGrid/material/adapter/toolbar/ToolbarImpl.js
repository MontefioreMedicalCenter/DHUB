...
export default class ToolbarImpl extends React.Component {
	...
	onPageChanged() {
		this.refresh()
		if (this.props.pager.doDispatchEvents) this.props.pager.dispatchEvent(new ExtendedFilterPageSortChangeEvent(ExtendedFilterPageSortChangeEvent.PAGE_CHANGE))
	...
	render() {
		...
		if (this.props.pager.level.getNestDepth() === 1) {
				...
				topLevelToolbarButtons.push(
					<span key="1">
						{grid.showAddEmployee && (
							<>
								<span key={gridId + 'btnAddEmployee'} className={'pagerDiv  iconCell'} id={gridId + 'btnAddEmployee'} onClick={this.executeToolbarAction.bind(this, 'Add Employee')}>
								...
								{grid.DocumentLibrary && (
									<span key={gridId + 'btnAddDocument'} className={'pagerDiv iconCell'} id={gridId + 'btnAddDocument'} onClick={this.executeToolbarAction.bind(this, 'Document Library')}>
										<Tooltip title="Document Library">
											<IconButton className={'imageButtonSize'} style={{ marginTop: '3px' }}>
						...
						{grid.showRefresh && (
							<span key={gridId + 'btnAddRefresh'} className={'pagerDiv  iconCell'} id={gridId + 'btnAddRefresh'} onClick={this.executeToolbarAction.bind(this, 'Refresh')}>
						...
						{grid.decrease && (
							<span key={gridId + 'btnCollapseOne'} className={'pagerDiv  iconCell'} id={gridId + 'btnCollapseOne'} onClick={grid.expandUp.bind(grid)}>
						...
						{grid.expand && (
							<span key={gridId + 'btnExpandOne'} className={'pagerDiv  iconCell'} id={gridId + 'btnExpandOne'} onClick={grid.expandDown.bind(grid)}>
			...
			if (grid.enableMultiColumnSort) {
				topLevelToolbarButtons.push(
					<span key="2">
						<span key={gridId + 'btnSort'} className={'pagerDiv  iconCell'} id={gridId + 'btnSort'} onClick={this.onMultiColumnSort.bind(this)}>
							<IconButton className={'imageButtonSize'} style={{ marginTop: '8px' }}>
								<img alt={Constants.PGR_BTN_SORT_TOOLTIP} className={'imageButtonSort'} src={sort} tabIndex={0} title={Constants.PGR_BTN_SORT_TOOLTIP} />
							</IconButton>
						</span>
			...
			if (grid.enablePreferencePersistence) {
				topLevelToolbarButtons.push(
					<span key="3">
						<span key={gridId + 'btnSettings'} className={'pagerDiv  iconCell'} id={gridId + 'btnSettings'} onClick={this.onShowSettingsPopup.bind(this)}>
				...
				if (grid.enableMultiplePreferences) {
					topLevelToolbarButtons.push(
						<span key="4">
							<span key={gridId + 'btnSettings'} className={'pagerDiv  iconCell'} id={gridId + 'btnSettings'} onClick={this.onOpenSettingsPopup.bind(this)}>
				...
				topLevelToolbarButtons.push(
					<span key="5">
						<span key={gridId + 'btnSaveSettings'} className={'pagerDiv  iconCell'} id={gridId + 'btnSaveSettings'} onClick={this.onSaveSettingsPopup.bind(this)}>
							<IconButton style={{ width: '40px', height: '40px' }} title={Constants.PGR_BTN_SAVE_SETTINGS_TOOLTIP}>
								<SettingsApplications className={'imageButtonSaveSettings'}>{}</SettingsApplications>
							</IconButton>
						</span>
						<span key={gridId + 'linesep' + linesep++} className={'pagerDiv lineSep'}>
							&nbsp;
			...
			if (this.props.pager.level.getEnableFilters()) {
				topLevelToolbarButtons.push(
					<span key="6">
						<span key={gridId + 'btnFilterShowHide'} className={'pagerDiv  iconCell'} id={gridId + 'btnFilterShowHide'} onClick={this.onShowHideFilter.bind(this)}>
							<IconButton style={{ width: '20px', height: '40px' }}>
								{/* filterShowHide */}
				...
				topLevelToolbarButtons.push(
					<span key="7">
						<span key={gridId + 'btnFilter'} className={'pagerDiv  iconCell'} id={gridId + 'btnFilter'} onClick={this.onProcessFilter.bind(this)}>
							<IconButton style={{ width: '20px', height: '40px' }}>
				...
				topLevelToolbarButtons.push(
					<span key="8">
						<span key={gridId + 'btnClearFilter'} className={'pagerDiv  iconCell'} id={gridId + 'btnClearFilter'} onClick={this.onClearFilter.bind(this)}>
							<IconButton style={{ width: '20px', height: '40px' }}>
			...
			if (grid.enablePrint) {
				topLevelToolbarButtons.push(
					<span key="9">
						<span key={gridId + 'btnPrint'} className={'pagerDiv  iconCell'} id={gridId + 'btnPrint'} onClick={this.onPrint.bind(this)}>
							<IconButton className={'imageButtonSize'}>
								<img alt={Constants.PGR_BTN_PRINT_TOOLTIP} className={'imageButtonPrint'} src={grid.getThemeToolbarIconFolder() + '/print.png'} tabIndex={0} title={Constants.PGR_BTN_PRINT_TOOLTIP} />
							</IconButton>
						</span>
					</span>
				)
			...
			if (grid.enablePdf) {
				topLevelToolbarButtons.push(
					<span key="10">
						<span key={gridId + 'btnPdf'} className={'pagerDiv  iconCell'} id={gridId + 'btnPdf'} onClick={this.onPdf.bind(this)}>
							<IconButton className={'imageButtonSize'}>
								<img alt={Constants.PGR_BTN_PDF_TOOLTIP} className={'imageButtonPdf'} src={grid.getThemeToolbarIconFolder() + '/pdf.png'} tabIndex={0} title={Constants.PGR_BTN_PDF_TOOLTIP} />
							</IconButton>
						</span>
					</span>
				)
			...
			if (grid.enableExport) {
				topLevelToolbarButtons.push(
					<span key="11">
						<span key={gridId + 'btnWord'} id={gridId + 'btnWord'}>
							<span key={gridId + 'btnExcel'} className={'pagerDiv  iconCell'} id={gridId + 'btnExcel'} onClick={this.onWordExport.bind(this)}>
								<Tooltip title={Constants.PGR_BTN_WORD_TOOLTIP}>
									<img tabIndex={0} src={word} className={'imageButtonExcel'} alt={Constants.PGR_BTN_EXCEL_TOOLTIP} style={{ marginTop: '9px' }} />
								</Tooltip>
							</span>
						</span>
					</span>
			...
			if (grid.enableExport) {
				topLevelToolbarButtons.push(
					<span key="12">
						<span key={gridId + 'btnExcel'} className={'pagerDiv  iconCell'} id={gridId + 'btnExcel'} onClick={this.onExcelExport.bind(this)}>
							<Tooltip title={Constants.PGR_BTN_EXCEL_TOOLTIP}>
								<img tabIndex={0} src={exporte} className={'imageButtonExcel'} alt={Constants.PGR_BTN_EXCEL_TOOLTIP} title={Constants.PGR_BTN_EXCEL_TOOLTIP} style={{ marginTop: '9px' }} />
							</Tooltip>
						</span>
					</span>
				)
		...
		const val = (
			<div className={'pagerControl flexiciousGridPager cellRenderer'} style={{ display: 'flex', overflow: 'auto', justifyContent: 'space-between' }}>
				<span
					...
					{this.props.pager.level.enablePaging ? (
						<span key="pageInfo" className={'pagerDiv pageInfo'}>
							{' '}
							{`${Constants.PGR_ITEMS} ${this.getPageStart()} ${Constants.PGR_TO} ${this.getPageEnd()} ${Constants.PGR_OF} ${this.getTotalRecords()}. ${Constants.PGR_PAGE} ${this.getPageIndex() + 1} ${Constants.PGR_OF} ${this.getPageCount()} `}
						</span>
					) : null}

					{this.props.pager.level.enablePaging ? (
						<span key={gridId + 'btnFirstPage'} className={'pagerDiv iconCell firstPage'} id={gridId + 'btnFirstPage'} onClick={this.onImgFirstClick.bind(this)}>
							<IconButton alt={Constants.PGR_BTN_FIRST_PAGE_TOOLTIP} className={'imageButtonFirstPage imageButtonSize'} disabled={this.getPageIndex() === 0} tabIndex="0" title={Constants.PGR_BTN_FIRST_PAGE_TOOLTIP} style={{ marginTop: '7px' }}>
								{' '}
								{/* <SkipPrevious color="primary" />{' '} */}
								{/* <img alt='firstPageArrow' src={firstPageArrow} /> */}
								<img tabIndex="0" src={firstPageArrow} className={'imageButtonFirstPage'} alt={Constants.PGR_BTN_FIRST_PAGE_TOOLTIP} title={Constants.PGR_BTN_FIRST_PAGE_TOOLTIP} />
							</IconButton>
					...
					{this.props.pager.level.enablePaging ? (
						<span key={gridId + 'btnPreviousPage'} className={'pagerDiv iconCell prevPage'} id={gridId + 'btnPreviousPage'} onClick={this.onImgPreviousClick.bind(this)}>
							<IconButton alt={Constants.PGR_BTN_PREV_PAGE_TOOLTIP} className={'imageButtonPrevPage imageButtonSize'} disabled={this.getPageIndex() === 0} tabIndex="0" title={Constants.PGR_BTN_PREV_PAGE_TOOLTIP} style={{ marginTop: '7px' }}>
								{' '}
					...
					{this.props.pager.level.enablePaging ? (
						<span key={gridId + 'btnNextPage'} className={'pagerDiv iconCell nextPage'} id={gridId + 'btnNextPage'} onClick={this.onImgNextClick.bind(this)}>
							<IconButton alt={Constants.PGR_BTN_NEXT_PAGE_TOOLTIP} className={'imageButtonNextpage imageButtonSize'} disabled={this.getPageIndex() >= this.getPageCount() - 1} tabIndex="0" title={Constants.PGR_BTN_NEXT_PAGE_TOOLTIP} style={{ marginTop: '7px' }}>
								{' '}
					...
					{this.props.pager.level.enablePaging ? (
						<span key={gridId + 'btnLastPage'} className={'pagerDiv iconCell lastPage'} id={gridId + 'btnLastPage'} onClick={this.onImgLastClick.bind(this)}>
							<IconButton alt={Constants.PGR_BTN_LAST_PAGE_TOOLTIP} className={'imageButtonLastPage imageButtonSize'} disabled={this.getPageIndex() >= this.getPageCount() - 1} tabIndex="0" title={Constants.PGR_BTN_LAST_PAGE_TOOLTIP} style={{ marginTop: '7px' }}>
								{' '}
					...
					{this.props.pager.level.enablePaging ? (
						<span key={gridId + 'lblGotoPage'} className={'pagerDiv  gotoPage'} id={gridId + 'lblGotoPage'}>
							{Constants.PGR_LBL_GOTO_PAGE_TEXT}{' '}
							<Select className={'pageDropdown'} value={this.props.pager._pageIndex + 1} onChange={this.onPageCbxChange.bind(this)}>
								{options.map((opt, i) => {
									return (
