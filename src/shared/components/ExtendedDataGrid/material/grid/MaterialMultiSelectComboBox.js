...
export default class MaterialMultiSelectComboBox extends MultiSelectComboBox {
	...
	onCheckDelayedChange(e) {
		...
		if (cbox.labelText === this.addAllItemText) this.allUnchecked = cbox.getSelectedState() === TriStateCheckBox.STATE_UNCHECKED
		...
		if (this.popup) {
			for (let i = 0; i < this.checkboxes.length; i++) {
				...
				if (this.allUnchecked) {
					...
					cb.setSelectedState(this.selectedValues.length >= this.getDataProvider().length - 1 ? TriStateCheckBox.STATE_CHECKED : this.selectedValues.length > 0 ? TriStateCheckBox.STATE_MIDDLE : TriStateCheckBox.STATE_UNCHECKED)
				} else {
