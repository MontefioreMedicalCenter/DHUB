...
export default class MaterialDateComboBox extends DateComboBox {
	...
	// 			item = dp[i]
	// 			options[options.length] = new Option(this.itemToLabel(item), UIUtils.resolveExpression(item, this.dataField))
	...
	// 	if (options.length > 0) {
	// 		this.children = [
	// 			<Select style={{ width: '100%' }} value={selectedValue || Constants.DEFAULT_ALL_ITEM_TEXT} onChange={this.handleChange}>
	// 				{options.map((opt, i) => {
	// 					return (
	...
	// 		const actions = [ToolbarAction.create(Constants.MCS_BTN_APPLY_LABEL, this.onDatePicker, true), ToolbarAction.create(Constants.MCS_BTN_CANCEL_LABEL, this.onDatePickerCancel.bind(this), true)]

	// 		this.popup = <DateRangePicker combo={this} dateFormatString={this.dateFormatString} grid={this.grid} />
