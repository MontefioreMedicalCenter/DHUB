 ...
 * @return {*}
 */
UIUtils.resolveExpression = (host, expression, valueToApply, returnUndefinedIfPropertyNotFound, applyNullValues, valueApplier) => {
	...
	if (UIUtils.expressionCache[expression].isSimple) {
		if (valueToApply != null || applyNullValues) {
			if (valueApplier) valueApplier.checkSetterAndApply(host, expression, valueToApply)
	...
	for (i = 0; i < fields.length; i++) {
		...
		if (field.includes('[')) {
			...
			if (endpointData.hasOwnProperty(field) || endpointData[field]) {
				if (valueToApply && i === fields.length - 1) endpointData[field] = valueToApply
...
Constants.GLOBAL_ROW_HEIGHT = 25 //so we can globally control a lot of different places.
